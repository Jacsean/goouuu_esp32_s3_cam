    ...
    : LcdDisplay(panel_io, panel, width, height)
{
    ...
    if (psram_size_mb >= 8)
    {
        ...
        ESP_LOGI(TAG, "Use 4MB of PSRAM(8MB) for image cache");
    }
    else if (psram_size_mb >= 2)
    {
        lv_image_cache_resize(512 * 1024, true);
        ESP_LOGI(TAG, "Use 512KB of PSRAM(8MB) for image cache");
    }
#endif
    ...
    ESP_LOGI(TAG, "Adding SPI LCD display");
    ...
    : LcdDisplay(panel_io, panel, width, height)
{
    ...
    ESP_LOGI(TAG, "Adding RGB LCD display");
    const lvgl_port_display_cfg_t display_cfg = {
        .io_handle = panel_io_,
        .panel_handle = panel_,
        .buffer_size = static_cast<uint32_t>(width_ * 20),
    ...
    : LcdDisplay(panel_io, panel, width, height)
{
    ...
    ESP_LOGI(TAG, "Adding Mipi LCD display");
...
void LcdDisplay::SetupUI()
{
    ...
    ESP_LOGD(TAG, ">>>>>>>>>>>>>>> WECHAT background size: %dx%d", LV_HOR_RES, LV_VER_RES);
...
void LcdDisplay::SetupUI()
{
    ...
    lv_obj_set_style_bg_color(container_, lvgl_theme->background_color(), 0);
    lv_obj_set_style_border_color(container_, lvgl_theme->border_color(), 0);
    ESP_LOGI(TAG, ">>>>>>>>>>>>>>> background size: %dx%d", LV_HOR_RES, LV_VER_RES);
    ...
    ESP_LOGI(TAG, ">>>>>>>>>>>>>>> preview_image_ size: %dx%d", width_, height_);
